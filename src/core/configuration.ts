      ...
      Utilities.forEachProperty(overwrite, (key: string, value: any) => {
        if (
          ...
          Utilities.forEachProperty(
            value,
            (key2: string, value2: any) => (result.paths![key2] = value2)
          );
        } else if (key === "config" && typeof result.config !== "undefined") {
          Utilities.forEachProperty(
            value,
            (key2: string, value2: any) => (result.config![key2] = value2)
          );
        } else {
  ...
  export class Configuration {
    ...
    private _createIgnoreDuplicateModulesMap(): void {
      ...
      for (let i = 0; i < this.options.ignoreDuplicateModules.length; i++) {
        this.ignoreDuplicateModulesMap[this.options.ignoreDuplicateModules[i]] =
          true;
      }
    }
    ...
    public moduleIdToPaths(moduleId: string): string[] {
      ...
      if (
        ...
        !Utilities.isAbsolutePath(result)
      ) {
