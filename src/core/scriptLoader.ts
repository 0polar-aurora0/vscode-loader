...
namespace AMDLoader {
  ...
  interface INodeVM {
    Script: {
      new (contents: string, options?: INodeVMScriptOptions): INodeVMScript;
    };
    ...
    public load(
      ...
      if (/^node\|/.test(scriptSrc)) {
        ...
        const vmScriptPathOrUri =
          this._getElectronRendererScriptPathOrUri(normalizedScriptSrc);
    ...
    private _handleCachedData(
      ...
      if (script.cachedDataRejected) {
        ...
        this._fs.unlink(cachedDataPath, (err) => {
          moduleManager
            .getRecorder()
            .record(LoaderEventType.CachedDataRejected, cachedDataPath);
        ...
        setTimeout(() => {
          ...
          this._fs.writeFile(
            ...
            (err) => {
              ...
              moduleManager
                .getRecorder()
                .record(LoaderEventType.CachedDataCreated, cachedDataPath);
              createLoop();
            }
          );
        }, timeout * 4 ** iteration++);
