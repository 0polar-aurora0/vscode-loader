...
namespace AMDLoader {
      ...
      if (this._callback) {
        if (typeof this._callback === "function") {
          ...
          if (
            ...
            (!this.exportsPassedIn || Utilities.isEmpty(this.exports))
          ) {
  ...
  export class ModuleManager {
     ...
     */
    //_modules2存储用于,如果一个模块已经存在于_modules中,它的代码已经被加载，但是不一定他的全部依赖被解析
     ...
     */
    //创建一个模块并存储在_mudules中，管理器会立即开始加载他的依赖
    public defineModule(
      strModuleId: string,
      dependencies: string[],
      callback: any,
      errorback: ((err: AnnotatedError) => void) | null | undefined,
      stack: string | null,
      moduleIdResolver: ModuleIdResolver = new ModuleIdResolver(strModuleId)
    ): void {
      //获取int类型id
      let moduleId = this._moduleIdProvider.getModuleId(strModuleId);
      //如果moduleId2中已经存在执行的模块id，跟据具体情况报错并中断模块定义
      ...
      if (this._config.isBuild()) {
        ...
        this._buildInfoDependencies[moduleId] = (m.dependencies || []).map(
          (dep) => this._moduleIdProvider.getStrModuleId(dep.id)
        );
      }
    ...
    private _normalizeDependencies(
      ...
      //由于依赖类型传入时有两种,在这里进行遍历操作对其进行统一处理
    ...
    public synchronousRequire(
      _strModuleId: string,
      moduleIdResolver: ModuleIdResolver = new ModuleIdResolver(_strModuleId)
    ): any {
      ...
      if (dependencies) {
        for (let i = 0, len = dependencies.length; i < len; i++) {
          ...
          if (this._hasDependencyPath(dependency.id, module.id)) {
            ...
            console.warn(
              cyclePath
                .map((id) => this._moduleIdProvider.getStrModuleId(id))
                .join(" => \n")
            );
          ...
          if (dependency instanceof PluginDependency) {
            ...
            let inversePluginDeps: PluginDependency[] =
              this._inversePluginDependencies2.get(dependency.pluginId);
